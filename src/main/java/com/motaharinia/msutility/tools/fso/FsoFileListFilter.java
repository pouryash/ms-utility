...
class FsoFileListFilter implements FileFilter {
    ...
    public boolean accept(File file) {
        ...
        boolean result;
        boolean acceptNameResult;
        if (!ObjectUtils.isEmpty(acceptNameArray)) {
            acceptNameResult = Arrays.stream(acceptNameArray).anyMatch(filename::startsWith);
        } else {
        ...
        if (!ObjectUtils.isEmpty(denyNameArray)) {
            denyNameResult = Arrays.stream(denyNameArray).noneMatch(filename::startsWith);
        }

        boolean denyExtensionResult = true;
        if (!ObjectUtils.isEmpty(denyExtensionArray)) {
            denyExtensionResult = Arrays.stream(denyExtensionArray).noneMatch(item -> filename.endsWith('.' + item));
        }
